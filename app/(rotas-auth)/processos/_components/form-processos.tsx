...
export default function FormProcessos() {
	...
	function onSubmit(values: z.infer<typeof formSchema>) {
		...
		startTransition(async () => {
			const parcelas = installment.map((item) => ({
				num_parcela: item.number,
				valor: item.value,
				vencimento: item.dueDate,
				cpf_cnpj: parcelasT.cpf_cnpj,
			...
			if (!resp.ok) {
				toast.error(resp.error ?? 'Algo deu errado');
			} else {
	...
	function handleCadastrarComParcelas() {
		if (!form.getValues('processo')) {
			toast.error('Informe o número do processo na aba Processo.');
			return;
		}
		if (installment.length === 0) {
			toast.error('Gere as parcelas antes de cadastrar.');
			return;
		}
					...
					<TabsContent
						...
						{installment.length > 0 && (
							<div>
								...
								<Button
									disabled={isPending || parcelasT.valorTotal == '' || !parcelasT.valorTotal}
									className='w-full mt-5'
									type='button'
									onClick={handleCadastrarComParcelas}>
